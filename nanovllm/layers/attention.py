...
# Try importing triton, but handle case when it's not available
...
def store_kvcache_pytorch(key: torch.Tensor, value: torch.Tensor, k_cache: torch.Tensor, v_cache: torch.Tensor, slot_mapping: torch.Tensor):
    # Reshape key and value tensors to combine head dimensions
    batch_size = key.shape[0]
    key_flat = key.reshape(batch_size, -1)
    value_flat = value.reshape(batch_size, -1)
    
    # Use advanced indexing to efficiently store the values
...
def store_kvcache(key: torch.Tensor, value: torch.Tensor, k_cache: torch.Tensor, v_cache: torch.Tensor, slot_mapping: torch.Tensor):
    ...
    if TRITON_AVAILABLE:
        ...
        # Use the PyTorch implementation when Triton is not available
